...
class Pengajian extends StatefulWidget {
  const Pengajian({Key? key}) : super(key: key);

  @override
  State<Pengajian> createState() => _PengajianState();
}

class _PengajianState extends State<Pengajian> {
  ...
  getPegajian() async {
    ...
    setState(() {
      tema = providerrekap.listNamaKegiatan?.first.temaPengajian;
  ...
  _kirim(String? hikmah) async {
    if (hikmah == '') {
      ...
      setState(() {
        _isLoading = false;
        Fluttertoast.showToast(
            msg: "Pilih lokasi presensi tidak boleh kosng",
            toastLength: Toast.LENGTH_SHORT,
            gravity: ToastGravity.CENTER,
            timeInSecForIosWeb: 1,
            backgroundColor: Colors.red,
      ...
      final _providerPresensi =
          Provider.of<ProviderPengajian>(context, listen: false);
      await _providerPresensi.kirimPengajian(
          hasilid ?? "", hikmahController.text);
      bool? status = _providerPresensi.status;
      String? message = _providerPresensi.message;
      if (status == true) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            backgroundColor: Color(0xFF0a4f8f), content: Text(message ?? "")));
        setState(() {
          _isLoading = _providerPresensi.isLoading;
        });
      } else {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                        ...
                        child: Container(
                            ...
                            child: Center(
                                child: Padding(
                                    ...
                                    child:
                                        Address == 'Lokasi Belum di Ketahui!!'
                                            //      ||
